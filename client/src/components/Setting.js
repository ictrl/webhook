...
export default function Settings() {
  ...
  const handleChangePhone = useCallback(
    (newNumber) => setAdminPhone(newNumber),
    []
  );

  const [senderID, setSenderID] = useState("ALERTS");
  const handleChangeSenderID = useCallback(
    (newValue) => setSenderID(newValue),
    []
  );

  const [orderCreateCustomer, setOrderCreateCustomer] = useState(false);
  const handleOrderCreateCustomer = useCallback(
    (value) => setOrderCreateCustomer(value),
    []
  );

  const [orderCreateAdmin, setOrderCreateAdmin] = useState(false);
  const handleOrderCreateAdmin = useCallback(
    (value) => setOrderCreateAdmin(value),
    []
  );

  const [orderCancelledCustomer, setOrderCancelledCustomer] = useState(false);
  const handleOrderCancelledCustomer = useCallback(
    (value) => setOrderCancelledCustomer(value),
    []
  );

  const [orderCancelledAdmin, setOrderCancelledAdmin] = useState(false);
  const handleOrderCancelledAdmin = useCallback(
    (value) => setOrderCancelledAdmin(value),
    []
  );

  const [orderFulfilledCustomer, setOrderFulfilledCustomer] = useState(false);
  const handleOrderFulfilledCustomer = useCallback(
    (value) => setOrderFulfilledCustomer(value),
    []
  );

  const [orderFulfilledAdmin, setOrderFulfilledAdmin] = useState(false);
  const handleOrderFulfilledAdmin = useCallback(
    (value) => setOrderFulfilledAdmin(value),
    []
  );
    ...
    <Fragment>
      <Form onSubmit={handleSubmit}>
        <div style={{ marginBottom: "3rem" }}>
          <Card sectioned>
            <h4>Rate :- 26 paisa per SMS only</h4>
          </Card>
        </div>
          ...
          >
            <Card sectioned>
              <div style={{ padding: "3rem" }}>
                ...
                />
                <small>
                  If you want to change sender id then email us to{" "}
                  <b>a.modi.abhishek@gmail.com</b>
                </small>
              </div>
            </Card>
          </Layout.AnnotatedSection>
            ...
            <Card sectioned>
              <div style={{ padding: "2rem" }}>
                ...
                >
                  ...
                  <Checkbox
                    label="Notify Admin"
                    checked={orderCreateAdmin}
                    onChange={handleOrderCreateAdmin}
                  />
                </div>
                <div
                  style={{ display: "flex", justifyContent: "space-between" }}
                >
                  ...
                  <Checkbox
                    label="Notify Admin"
                    checked={orderCancelledAdmin}
                    onChange={handleOrderCancelledAdmin}
                  />
                </div>
                ...
                >
                  ...
                  <Checkbox
                    label="Notify Admin"
                    checked={orderFulfilledAdmin}
                    onChange={handleOrderFulfilledAdmin}
                  />
                </div>
              </div>
            </Card>
