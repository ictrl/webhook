	...
	if (validUrl.isUri(longUrl)) {
		try {
			...
			if (url) {
				...
				await Store.findOne({ name: shop, abandanTemplate: { $elemMatch: { topic: followUp } } }, (err, data) => {
					if (err) {
						console.log(err);
					} else {
				...
				const store = new Store({
					...
					template: [
						...
						},
						{
							...
							customer: '`hello%20${name}%20${price}canc`',
							admin: '`hello%20${name}%20orders/clc%20admin`'
						},
						{
							topic: 'orders/fulfilled',
							customer: '`hello%20${name}%20${price}ful`',
		...
		if (!err) {
			...
			if (data.smsCount > 0) {
				...
				var options = {
					...
					port: null,
					path: `/api/sendhttp.php?mobiles=${phone}&authkey=${process.env
